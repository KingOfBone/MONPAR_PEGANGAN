<?php
				...
				foreach($FILES as $key=>$f) {
					...
					if(ucwords($nama_table) == 'Pengisian') {
						if(!empty($POST['Kode_Apar']) && empty($POST['kode_pengisian']))
							$sql = "select lantai, no_apar, penempatan, id_apar 'kode_apar' from apar where id_apar = $POST[Kode_Apar]";
						else if(empty($POST['Kode_Apar']) && !empty($POST['kode_pengisian']))
							$sql = "select lantai, no_apar, penempatan, kode_apar from pengisian inner join apar on pengisian.kode_apar=apar.id_apar where kode_pengisian = $POST[kode_pengisian]";
						...
						$sql = "select lantai, no_apar, penempatan, kode_apar, foto_tacometer from pengisian inner join apar on pengisian.kode_apar=apar.id_apar where kode_pengisian = $POST[kode_pengisian]";
					...
					$id_apar = !empty($row_apar) ? $row_apar['kode_apar'] : null;
					$id_apar = !empty($POST['Kode_Apar']) ? $POST['Kode_Apar'] : $id_apar;
					
					if(empty($id_apar)) {
						$sql = "select id_apar from apar order by id_apar desc";
					...
					$id_apar = !empty($id_apar) ? "-$id_apar" : null;
					...
					//$newfilename="$lokasi Lantai $lantai_apar - $no_apar_apar - $penempatan_apar_apar $id_apar".".".$extension;
					$newfilename = $lokasi."-$jenisuser-Lantai $lantai_apar-$no_apar_apar-$penempatan_apar_apar$id_apar".".".$extension;
	...
	function insert() {
		...
		if(in_array($nama_table, $table_array)) {
			...
			foreach($FILES as $key=>$f) {
				$id_apar = !empty($POST['No_Apar']) ? $POST['no_apar'] : null;
				$id_apar = !empty($POST['kode_pengisian']) ? $POST['kode_pengisian'] : $id_apar;
				$id_apar = !empty($POST['Kode_Apar']) ? $POST['Kode_Apar'] : $id_apar;
				...
				if($nama_table == 'Pengisian') {
					$sql = "select * from apar where id_apar = $id_apar";
					$sql = mysql_query($sql);
					$row_apar = mysql_fetch_array($sql);
				}
				else if($nama_table == 'Pemeliharaan') {
					...
					$id_apar = "$id_apar-Pengisian $row_apar[kode_pengisian]-Pemeliharaan $kode_pemeliharaan";
				}
				...
				//$id_apar = !empty($POST['Kode_Apar']) ? "- ".$POST['Kode_Apar'] : null;
	...
	function multiple_query_form_update() {
		...
		if($nama_table == 'apar') {
			...
			$q = [
				...
				"id_apar"=>"select id_apar 'id', concat(tipe, ' - ', model) 'val' from APAR"
			];
		}
		else if($nama_table == 'pengisian') {
			$q = [
				"kode_apar"=>"
					select id_apar 'id', concat('Lantai ', lantai, ' / ', no_apar, ' / ', penempatan) 'val' from APAR 
					where
	...
	function kumpulan_query_tampil_dibaris_awal() {
		...
		if($nama_table == 'Apar') {
			...
			$q = "
				select 
					concat('Lantai', lantai, ' / ', no_apar, ' / ', penempatan) 'as_napar', tgl_pengisian_terakhir, tgl_pengisian_kembali, catatan, Apar.gambar_apar, $pk 'id', Apar.id_apar 'fk' from $nama_table 
				inner join Apar 
					on $nama_table.kode_apar=apar.id_apar 
				$klausa 
				order by $pk desc 
			";
			/* $q = "
			...
			$q = "select $nama_table.*, Apar.no_apar, Apar.penempatan, Jenis_Api.jenis_api, Pengisian.tgl_pengisian_terakhir, $pk 'id', Pengisian.Kode_Pengisian 'fk' from $nama_table inner join Pengisian on $nama_table.Kode_Pengisian=Pengisian.Kode_Pengisian inner join Apar on Pengisian.Kode_Apar=Apar.id_apar inner join Jenis_Api on Apar.id_jenis_api=Jenis_Api.id_jenis_api $klausa order by $pk desc ";
		}
		else if($nama_table == 'Pemakaian') {
			$klausa = "where kantor = '$jenisuser' AND kode_kantor = $kode_kantor";
			$q = "select $nama_table.*, $pk 'id', Apar.id_apar 'fk' from $nama_table inner join Apar on $nama_table.kode_apar=Apar.id_apar $klausa order by $pk desc ";			
		}
		else if($nama_table == 'Dashboard') {
			$q['3bulan'] = "
				select apar.no_apar, apar.penempatan, concat(namaapp, ' - ', namagi) 'unit', tgl_pengisian_terakhir from `master`.app 
				inner join `master`.gi
					on `master`.app.kodeapp=`master`.gi.kodeapp
				inner join apar.apar
					on apar.apar.kode_gi=`master`.gi.kodegi
				inner join apar.pengisian
					on apar.pengisian.kode_apar=`apar`.apar.id_apar
				where tgl_pengisian_terakhir <= now()-interval 3 month
			";
			$q['1bulan'] = "select Apar.no_apar, Apar.penempatan, tgl_pengisian_terakhir, $pk 'id', Apar.id_apar 'fk' from Apar inner join Pengisian on Pengisian.kode_apar=Apar.id_apar where tgl_pengisian_terakhir <= now()-interval 1 month order by $pk desc ";			
			$q['1minggu'] = "select Apar.no_apar, Apar.penempatan, tgl_pengisian_terakhir, $pk 'id', Apar.id_apar 'fk' from Apar inner join Pengisian on Pengisian.kode_apar=Apar.id_apar where tgl_pengisian_terakhir <= now()-interval 1 week order by $pk desc ";			
