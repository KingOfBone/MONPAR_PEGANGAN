					...
					<form id="validate-me-plz" name="form1" enctype="multipart/form-data" role="form" action="#" method="post">
						<?php
							...
							foreach($kolom_table as $key=>$kol) {
								...
								$cek_hidden_isi = strpos($kol['Komen'], 'hidden_isi');
						...
						<?php
									...
									if($cek_primary !== false) {
										...
										$value = '';
										if($cek_hidden_isi !== false) {
											if($kolom_asli == 'kantor')
												$value = $_SESSION['jenisuser'];
											else if($kolom_asli == 'kode_kantor')
												$value = $_SESSION["kode".$_SESSION['jenisuser']];												
										}
										else 
											$value = $ses_kodelogin;
										
										echo "<input type='hidden' name='$kolom_asli' value='$value'/>";
											...
											<div class="row">
												...
												<div class="col-md-5">
													...
													<?php 
															...
															$objek = $kolom_asli == 'foto_tacometer' ? 'tacometer' : $objek;
															$objek = $kolom_asli == 'foto_stiker' ? 'stiker' : $objek;
															
															$lihat_peta = lihat_peta($nama_table, $row, $objek);
															$img_src = $lihat_peta[0];
															
													?>
															<input type="file" class="form-control" id='gambar_dinamis_<?php echo $i_gambar_dinamis; ?>' name="<?php echo $kolom_asli; ?>" value="<?php echo $row[$kol['Kolom']]; ?>" />
															<img id="blah_<?php echo $i_gambar_dinamis; ?>" src="<?php echo $img_src; ?>" alt="" class='rotateimg90' style='width:150px; height:150px;' />
