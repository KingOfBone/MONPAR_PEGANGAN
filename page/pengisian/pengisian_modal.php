<?php
	...
	$sql = "select $nama_table.*, Apar.*, $pk 'id', Apar.id_apar 'fk' from $nama_table inner join Apar on $nama_table.kode_apar=apar.id_apar order by $pk desc limit 0, 30";
...
<div class="row">
    ...
    <div class="col-md-3">
        <?php 
			...
			$gambar = $lihat_peta[0];
			$nama_gambar = "APAR Lantai $row[lantai] / $row[no_apar] / $row[penempatan]";
		?>
		...
		<?php 
			$lihat_peta = lihat_peta($nama_table, $row, 'stiker');
			
			$gambar = $lihat_peta[0];
			$nama_gambar = "Stiker Lantai $row[lantai] / $row[no_apar] / $row[penempatan]";
		?>
		...
		<?php 
			$lihat_peta = lihat_peta($nama_table, $row, 'lokasi');
